...
| GET | `/search?q=&limit=&offset=&type=&category=&tag=&status=&fullText=` | – | `SearchResult` — keyword+semantic hybrid; `limit`/`offset` paginate (`total` returns the full match count before paging); optional filters `type` (`fact`/`decision`/…), `category` (label), `tag` (exact), `status` (`pending`/`reviewed`/…), `fullText=1` (match inside full body beyond title/summary/tags); each item includes matched `terms` for UI highlighting |
